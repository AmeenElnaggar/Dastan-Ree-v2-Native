export function renderBlogCard(blog) {
  ...
  const fallbackReadTimes = [5, 4, 6, 3, 5, 4];
  const readTime =
    typeof blog.readTime === "number"
      ? blog.readTime
      : fallbackReadTimes[(id - 1) % fallbackReadTimes.length];
  const [monthFull, dayComma] = date.split(" ");
  const day = dayComma.replace(",", "");
  const month = monthFull.slice(0, 3).toUpperCase();
  const detailsUrl = `../../pages/blog-details/index.html?id=${id}`;

  return `
    <article class="blog-card">
      <a class="blog-card__image-wrap" href="${detailsUrl}" aria-label="${title}">
      ...
      <div class="blog-card__body">
        ...
        <h3 class="blog-card__title">
          <a href="${detailsUrl}" class="blog-card__title-link">${title}</a>
        </h3>
        ...
        <div class="blog-card__footer">
          ...
          <a href="${detailsUrl}" class="blog-card__cta">
            <span>Read More</span>
